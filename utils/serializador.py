...
def serializar_historico(h: HistoricoAcademico) -> dict:
    return {
        ...
        "registros_disciplinas": [
            r.to_dict() for r in (h.get_registros_disciplinas() or [])
        ]
    }
...
def serializar_matricula(matricula: Matricula) -> dict:
    return {
        ...
        "aluno_matricula_id": matricula.get_aluno().get_matricula()
                              if matricula.get_aluno() else None,
        "historico_academico_id": (
            matricula.get_historico_academico().get_id()
            if matricula.get_historico_academico() and hasattr(matricula.get_historico_academico(), "get_id")
            else None
        ),
        "registros_disciplinas": [
            r.to_dict() for r in (matricula.get_registros_disciplinas() or [])
        ]
    }
...
def serializar_matricula_paga_disciplina(mpd: MatriculaPagaDisciplina) -> dict:
    ...
    return {
        ...
        "historico_id": (
            ...
            if mpd.get_historico_acad() else None
        ),
        "notas": mpd.get_notas(),
